...
function Module:SetCombatMode(mode)
    ...
    if self.ClassConfig then
        if self.ClassConfig.BeforeResolveActions then
            Core.SafeCallFunc("BeforeResolveActions", self.ClassConfig.BeforeResolveActions, self)
        end
...
function Module:RunHealRotation()
    ...
    local healGate = self:GetHealScanThreshold()
    self:HealById(Combat.FindWorstHurtGroupMember(healGate))

    if Config:GetSetting('UseHealList') then
        self:HealById(Combat.FindWorstHurtHealList(healGate))
    else
        self:HealById(Combat.FindWorstHurtXT(healGate))
    end
end

--- Highest configured heal/HoT threshold used as the who-to-heal scan gate.
--- Replaces the old MaxHealPoint setting: if any heal could fire, we must scan by then.
---@return number
function Module:GetHealScanThreshold()
    local maxPct = 0
    local function consider(val)
        local v = tonumber(val) or 0
        if v > maxPct then maxPct = v end
    end

    -- Class HealPct* (e.g. EQ Might CLR): use only these so unused core Light/Main points
    -- do not inflate the scan gate. Legacy CompleteHealPct is not scanned.
    local hasClassHealPcts = false
    if self.ClassConfig and self.ClassConfig.DefaultConfig then
        for key, _ in pairs(self.ClassConfig.DefaultConfig) do
            if type(key) == "string" and key:find("^HealPct") then
                hasClassHealPcts = true
                consider(Config:GetSetting(key))
            end
        end
    end

    if not hasClassHealPcts then
        for _, key in ipairs({ "LightHealPoint", "MainHealPoint", "BigHealPoint", "GroupHealPoint", "PetHealPoint", }) do
            consider(Config:GetSetting(key))
        end
    end

    if maxPct <= 0 then return 100 end
    if maxPct > 100 then return 100 end
    return maxPct
end

--- True if anyone we heal (group/pets + heal list or XT) is below Heal Priority Point this frame.
---@return boolean
function Module:NeedToHeal()
    local point = Config:GetSetting('HealPriorityPoint') or 0
...
function Module:GiveTime()
    ...
    if (self:IsRezing() or self:HasRezClickies()) and Config:GetSetting('DoRez') and Core.OkayToNotHeal(true) then
        if not (combat_state == "Downtime" and mq.TLO.Me.Invis() and not Config:GetSetting('BreakInvisForHealing')) then
                ...
                if targetTable and #targetTable > 0 then
                    if Core.SafeCallFunc(string.format("Rotation Condition Check for %s", r.name), r.cond, self, combat_state) then
                        ...
                        local newState = Rotation.Run(self, self:GetRotationTable(r.name), targetTable,
                            ...
                            self:GetRotationEntryToggles(r.name), r.IgnoreImmuneCheck)
