...
return {
        ...
        },
        {
            ...
            cond = function(self, combat_state)
                return combat_state == "Combat" and mq.TLO.Me.PctAggro() > Config:GetSetting('JoltAggro') and Casting.OkayToCombatEscape()
                    and (not Config:GetSetting('JoltNamedOnly') or Globals.AutoTargetIsNamed)
            end,
        },
    ...
    ['Helpers']           = {
        rangedNav = function(reason)
            ...
            -- "You can't hit them from here.": close if out of BowRange, else circle for a clear shot angle.
            if reason == "canthit" then
                if Targeting.GetTargetDistance() > bowRange + 10 then
                    reason = "toofar"
                else
                    reason = "cantsee"
                end
            ...
            -- CantSee / blocked shot: walk toward the target (no lineofsight=on) until game LoS returns.
            -- NavAroundCircle often fails when MQ LoS TLO disagrees with the game's "cannot see" message.
            if reason == "cantsee" then
                local tid = Globals.AutoTargetID
                local now = mq.gettime()
                if Globals.LastCantSeeNavMs and (now - Globals.LastCantSeeNavMs) < 3000 then
                    return true
                end
                Globals.LastCantSeeNavMs = now

                local targDist = Targeting.GetTargetDistance() or 0
                local holdDist = (bowRange and bowRange > 0) and bowRange or 30
                local desiredDistance = math.floor(math.max(10, math.min(targDist > 5 and (targDist * 0.6) or holdDist, holdDist)))
                ...
                Logger.log_info("Ranged nav: CantSee — naving to id %d distance=%d until LoS", tid, desiredDistance)
                Core.DoCmd("/squelch /face fast")
                Movement:DoNav(false, "id %d distance=%d log=off", tid, desiredDistance)
                mq.delay(500, function() return mq.TLO.Navigation.Active() end)

                if mq.TLO.Navigation.Active() then
                    mq.delay(5000, function()
                        return (mq.TLO.Target.ID() or 0) == 0
                            or mq.TLO.Target.LineOfSight()
                            or not mq.TLO.Navigation.Active()
                    end)
                    if mq.TLO.Navigation.Active() and mq.TLO.Target.LineOfSight() then
                        Movement:DoNav(false, "stop log=off")
                        Movement.LastDoNavCmd = ""
                    end
                    Core.DoCmd("/squelch /face fast")
                else
                    Logger.log_warn("Ranged nav: CantSee — no nav path, stick until LoS")
                    Movement:DoStickCmd("%d id %d moveback uw", holdDist, tid)
            ...
            if Config:GetSetting('UseRangedStick') then -- Use Ranged Stick: hold bow range (or Stick Distance) with a stick.
                local rangedDist = Movement.GetRangedStickDistance()
                if reason == "toofar" or Targeting.GetTargetDistance() > rangedDist + 10 then
                    if not mq.TLO.Navigation.Active() then
                        Movement:DoNav(true, "id %d distance=%d lineofsight=on", Globals.AutoTargetID, rangedDist)
                        Core.DoCmd('/squelch /face fast')
                    end
                elseif (mq.TLO.Stick.StickTarget() or 0) ~= Globals.AutoTargetID or (mq.TLO.Stick.Status() or "off"):lower() == "off" then
                    ...
                    Movement:DoStickCmd("%d id %d %s", rangedDist, Globals.AutoTargetID, stickArgs)
                end
        ...
        ['Aggro Management']   = {
            {
                ...
                cond = function(self, spell, target)
                    return not Config:GetSetting('JoltNamedOnly') or Globals.AutoTargetIsNamed
                end,
            },
        },
        ['Combat']             = {
    ...
    ['DefaultConfig']     = { --TODO: Condense pet proc options into a combo box and update entry conditions appropriately
        ...
        ['BowRange']        = {
            ...
            Category = "Archery",
            Index = 101,
            Tooltip = "Preferred archery distance for reposition / LoS fixes.\n" ..
                "Also used by Ranged Stick; set to 0 to fall back to Stick Distance (blank = auto 14/34 by size).",
            Default = 15,
            Min = 0,
            Max = 300,
        ...
        ['UseRangedStick']  = {
            ...
            Warning = function()
                ...
                if Config:GetSetting('ChaseOn') then
                    if Config:GetSetting('ChaseDistance') < rangedDist then
                        return true, "Warning: Chase Distance is below ranged stick distance - chase may fight the ranged stick hold."
                    end
                elseif Config:GetSetting('ReturnToCamp') and Config:GetSetting('CampLeashCombat') and Config:GetSetting('AutoCampRadius') < rangedDist then
                    return true, "Warning: Camp Radius is below ranged stick distance - Leash to Camp (Combat) may fight the ranged stick hold."
                end
                return false, ""
            end,
            FAQ = "Why is my ranger rubber-banding, charging back and forth or changing heading constantly?",
        ...
        ['JoltNamedOnly'] = {
            DisplayName = "Jolt on Named Only",
            Group = "Abilities",
            Header = "Utility",
            Category = "Hate Reduction",
            Index = 103,
            Tooltip = "When enabled, cast Jolt only on Named targets (still requires high aggro / Use Jolt Spell).",
            Default = false,
        },
        ['DoCoverTracks'] = {
