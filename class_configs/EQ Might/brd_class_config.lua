...
local Tooltips     = {
    ...
    MagicDotSong   = "Song Line: Magic DoT and magic resist debuff (Fufil's)",
    ErvajSong      = "Song: Ervaj's Lost Composition — ST mana drain (BRD 60)",
    EndBreathSong  = "Song Line: Enduring Breath",
    SustenanceSong = "Song Line: Group Food/Water (Song of Sustenance)",
    ...
    ResistSong     = "Song Line: Damage Shield / Group Resist Increase (or selected Low Level Psalm)",
...
-- Resist Psalm: default high-level line, or force a classic / chorus song by choice.
local resistSongHighLevel = {
    "Psalm of Veeshan", -- Level 63
}

-- Combo index 1 = High Level (Veeshan); 2+ map to these songs.
local lowLevelPsalmByChoice = {
    [2] = "Psalm of Warmth",           -- Level 25, Cold resist + DS
    [3] = "Psalm of Vitality",         -- Level 29, Disease resist
    [4] = "Psalm of Cooling",          -- Level 33, Fire resist + DS
    [5] = "Psalm of Purity",           -- Level 37, Poison resist
    [6] = "Psalm of Mystic Shielding", -- Level 41, Magic resist
    [7] = "Elemental Chorus",          -- Level 54, Magic/Fire/Cold resist
    [8] = "Purifying Chorus",          -- Level 56, Poison/Disease/Magic resist + AC
}
...
local function ApplyResistPsalmChoice()
    local choice = Config:GetSetting('ResistPsalmChoice') or 1
    if type(choice) ~= "number" or choice < 2 then
        _ClassConfig.AbilitySets['ResistSong'] = copySpellList(resistSongHighLevel)
        return
    end

    local song = lowLevelPsalmByChoice[choice]
    _ClassConfig.AbilitySets['ResistSong'] = song and { song, } or copySpellList(resistSongHighLevel)
end

_ClassConfig = {
    _version              = "3.6 - EQ Might",
    _author               = "Algar, Derple, Grimmier, Tiddliestix, SonicZentropy",
    ['BeforeResolveActions'] = function()
        ApplyResistPsalmChoice()
    end,
    ...
    ['AbilitySets']       = {
        ...
        ['SustenanceSong'] = {
            "Song of Sustenance", -- Level 15, group food/water
        },
        ...
        ['MagicDotSong'] = {
            "Fufil's Diminishing Dirge", -- Level 60, Magic DoT + MR debuff
            "Fufil's Curtailing Chant",  -- Level 30, Magic DoT + MR debuff (EQM id 707)
        },
        ['ErvajSong'] = {
            "Ervaj's Lost Composition", -- Level 60, ST mana drain (spell id 2936)
        },
        ['CureSong'] = {
        ...
        ['ResistSong'] = { -- Rewritten by BeforeResolveActions from ResistPsalmChoice
            "Psalm of Veeshan", -- Level 63
        },
        ['MezSong'] = {
        ...
        ['CalmSong'] = {
            -- CalmSong - Level Range 8+ -- optional mem for manual /rgl usemap (UseCalmSong)
        ...
        ['Puretone'] = {
            "Puretone Discipline", -- Level 60, instrument modifier for songs
        },
        ['Steelwrath'] = {
        ...
        ['MitigationSong'] = {
            ...
            "Niv's Harmonic",             -- Level 58, group AC / SpellShield (id 1763)
            "Nillipus' March of the Wee", -- Level 52, AC/AGI + absorb (EQM id 1752)
        },
        ['Protective'] = {
        ...
        ['Melody']            = {
            {
                name = "SustenanceSong",
                type = "Song",
                load_cond = function(self) return Config:GetSetting('UseSustenance') > 1 end,
                cond = function(self, songSpell)
                    return self.Helpers.CheckSongStateUse(self, "UseSustenance") and self.Helpers.RefreshBuffSong(self, songSpell)
                end,
            },
            {
                name = "AreaAriaSong",
            ...
            },
            {
                name = "MagicDotSong",
                type = "Song",
                load_cond = function(self) return Config:GetSetting('UseMagicDots') end,
                cond = function(self, songSpell, target)
                    return target.ID() ~= mq.TLO.Me.ID() and self.Helpers.DotSongCheck(songSpell) and
                        self.Helpers.GetDetSongDuration(songSpell) <= Config:GetSetting('SongRefresh')
                end,
            },
            {
                name = "ErvajSong",
                type = "Song",
                load_cond = function(self) return Config:GetSetting('UseErvajSong') end,
                cond = function(self, songSpell, target)
    ...
    ['SpellList']         = { -- New style spell list, gemless, priority-based. Will use the first set whose conditions are met.
        {
            ...
            spells = {
                ...
                { name = "MezAESong",      cond = function(self) return Config:GetSetting('MezOn') and Config:GetSetting('DoAEMez') end, },
                { name = "MezSong",        cond = function(self) return Config:GetSetting('MezOn') and Config:GetSetting('DoSTMez') end, },
                { name = "CharmSong",      cond = function(self, spell) return Core.IsSelectedCharmSpell(spell) end, }, -- mem only when Charm Ability is this song (not a clicky/AA)
                ...
                { name = "SustenanceSong", cond = function(self) return Config:GetSetting('UseSustenance') > 1 end, },
                -- major group buffs
                ...
                { name = "MagicDotSong",   cond = function(self) return Config:GetSetting('UseMagicDots') end, },
                { name = "ErvajSong",      cond = function(self) return Config:GetSetting('UseErvajSong') end, },
                -- filler
                { name = "CalmSong",       cond = function(self) return Config:GetSetting('UseCalmSong') end, },
            },
        },
    },
    ['PullAbilities']     = {
    ...
    ['DefaultConfig']     = {
        ...
        ['UseSustenance']   = {
            DisplayName = "Use Song of Sustenance",
            Group = "Abilities",
            Header = "Buffs",
            Category = "Group",
            Index = 103,
            Tooltip = Tooltips.SustenanceSong,
            Type = "Combo",
            ComboOptions = { 'Never', 'In-Combat Only', 'Always', 'Out-of-Combat Only', },
            Default = 1,
            Min = 1,
        ...
        ['UseCalmSong']     = {
            DisplayName = "Mem Calm Song",
            Group = "Abilities",
            Header = "Debuffs",
            Category = "Calm",
            Index = 101,
            Tooltip = "Memorize CalmSong for manual pacify (/rgl usemap song CalmSong). Not used in Melody.",
            Default = false,
            RequiresLoadoutChange = true,
            FAQ = "Why memorize Calm Song?",
            Answer = "Calm is not sung automatically. Enable this only if you want it on a gem for manual use via /rgl usemap.",
        },
        ...
        ['ResistPsalmChoice'] = {
            DisplayName = "Low Level Psalm",
            Group = "Abilities",
            Header = "Buffs",
            Category = "Group",
            Index = 105,
            Tooltip = "Which resist song to use when Use Resist Buff is enabled.\n" ..
                "None: Psalm of Veeshan.\n" ..
                "Attribute/Chorus: force that song (Warmth/Vitality/Cooling/Purity/Mystic Shielding/Elemental Chorus/Purifying Chorus).",
            Type = "Combo",
            ComboOptions = {
                'None (Veeshan)',
                'Cold (Warmth)',
                ...
                'Fire (Cooling)',
                'Poison (Purity)',
                'Magic (Mystic Shielding)',
                'Elemental Chorus (Magic/Fire/Cold)',
                'Purifying Chorus (Poison/Disease/Magic)',
            },
            Default = 1,
            Min = 1,
            Max = 8,
            RequiresLoadoutChange = true,
            FAQ = "How do Low Level Psalms work?",
            Answer = "Set Use Resist Buff to when you want the song (e.g. Always), then pick Low Level Psalm.\n" ..
                "Cold=Warmth (25), Disease=Vitality (29), Fire=Cooling (33), Poison=Purity (37),\n" ..
                "Magic=Mystic Shielding (41), Elemental Chorus (54 Magic/Fire/Cold),\n" ..
                "Purifying Chorus (56 Poison/Disease/Magic).\n" ..
                "None keeps Psalm of Veeshan (63).",
        },
        ['UseMitigation']   = {
        ...
        ['UseMagicDots']    = {
            DisplayName = "Use Magic Dots",
            Group = "Abilities",
            Header = "Damage",
            Category = "Over Time",
            Index = 105,
            Tooltip = Tooltips.MagicDotSong,
            RequiresLoadoutChange = true,
            Default = false,
        },
        ['UseErvajSong']    = {
            DisplayName = "Use Mana Damage",
            Group = "Abilities",
            Header = "Damage",
            Category = "Direct",
            Index = 101,
            Tooltip = Tooltips.ErvajSong,
            RequiresLoadoutChange = true,
            Default = false,
        },
        ['UseJonthan']      = {
