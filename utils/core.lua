...
--- Safe to skip healing this frame.
---@param checkPriority boolean? true = honor HealPriorityPoint (0 disables). nil/false = do not yield for heals.
---@return boolean True if it is safe to perform non-heal actions.
function Core.OkayToNotHeal(checkPriority)
    ...
    if not checkPriority then return true end
    return not Modules:ExecModule("Class", "NeedToHeal")
end
...
function Core.CombatActionsCheck()
    ...
    if not Core.OkayToNotHeal(true) then return false end
